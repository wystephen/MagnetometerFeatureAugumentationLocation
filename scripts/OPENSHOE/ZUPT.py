# -*- coding:utf-8 -*-
# carete by steve at  2016 / 11 / 19　16:02
...
class ZUPTaidedInsPlus(object):
    ...
    def init_Nav_eq(self, u1):
        ...
        tmp_acc = np.zeros(3)
        # for tmp_acc_i in range(3):
        #     tmp_acc[tmp_acc_i] = np.mean(u2[tmp_acc_i,:])
        tmp_acc[0] = f_u
        tmp_acc[1] = f_v
        tmp_acc[2] = f_w

        print('u1:', tmp_acc, 'norm is :', np.linalg.norm(tmp_acc))
        print('rt2b:', (Rb2t.dot(tmp_acc)), 'norm is :', np.linalg.norm(Rb2t.dot(tmp_acc)))
